...
async function runViewport(name, width, height, deviceScaleFactor = 1, mobile = false) {
  ...
  const metrics = await send('Runtime.evaluate', { expression: `(() => ({
    ...
    elements: Array.from(document.querySelectorAll('*')).map(el => {
      ...
      return { tag: el.tagName, text: (el.innerText || '').replace(/\s+/g, ' ').trim().slice(0, 100), color: s.color, backgroundColor: s.backgroundColor, fontFamily: s.fontFamily, fontSize: s.fontSize, fontWeight: s.fontWeight, lineHeight: s.lineHeight, borderRadius: s.borderRadius, padding: s.padding, margin: s.margin, x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) };
    }).filter(x => x.w > 0 && x.h > 0).slice(0, 260)
  }))()`, returnByValue: true });
